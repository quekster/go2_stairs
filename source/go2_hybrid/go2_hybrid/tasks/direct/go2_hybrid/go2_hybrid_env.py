...
class Go2HybridEnv(DirectRLEnv):
    ...
    def _get_observations(self) -> dict:
        #height_obs gives vertical distance between the scanner origin and the hit point in world coordinates
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        base_origin[:, 0] = base_origin[:, 0] + self.x_offset_b
        # lift robot slightly so it’s not intersecting the mesh
        base_origin[:, 2] = base_origin[:, 2] +  self.z_offset_b
    ...
    def resample_commands(self, env_ids: torch.Tensor):
        ...
        """Phase 0: Flat Ground - Random 2D velocity + heading """
        ...
        # """Phase 1: Stair Climbing - Forward speed + heading towards stairs only """
        ...
        # speed = torch.empty(num_envs, device=self.device).uniform_(0.7, 1.2)
    ...
    def plot_lidar_3d(self, env_id=0, frame="base"):
        """
        Plot the current LiDAR hits for env_id in either:
            - base frame  (default, recommended)
            - world frame
        No temporal stacking. Only a single frame.
        """
        ...
        if frame == "base":
          hits = self.get_bf_hits(torch.tensor([env_id], device=self.device))[0]  # [R, 3]
        elif frame == "world":
            hits = self._lidar_scanner.data.ray_hits_w[env_id]                      # [R, 3]
        else:
            raise ValueError(f"Invalid frame '{frame}'. Use 'base' or 'world'.")
        # Convert to NumPy
        hits = hits.detach().cpu().numpy()
        hits = np.nan_to_num(hits, nan=self._lidar_range, posinf=self._lidar_range, neginf=-self._lidar_range)

        # Create 3D scatter
        fig = plt.figure(figsize=(8, 6))
        ax = fig.add_subplot(111, projection="3d")

        # Un-normalize for visualization (if needed)
        # If your hits_b are in meters already, comment this out.
        # Note: get_bf_hits returns actual meters → do NOT multiply by range again.
        # hits = hits * self._lidar_range
        ...
        ax.set_ylabel("Y (m)")
        ax.set_zlabel("Z (m)")
        ax.set_title(f"LiDAR hits ({frame}-frame) – Env {env_id}, Step {self._step_counter}")

        if hits_valid.shape[0] > 0:
            max_range = np.ptp(hits_valid, axis=0).max() / 2.0
            mid = np.mean(hits_valid, axis=0)
